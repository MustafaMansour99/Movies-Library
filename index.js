...
function trendingHandler(req, res) {
  try {
    ...
    axios
      .get(url)
      .then((result) => {
        //<<= like await and async
        let mapRes = result.data.results.map((item) => {
          let singleMovie = new Movielibrary(
            item.id,
            item.title,
            item.release_date,
            item.poster_path,
      ...
      .catch((error) => {
        //if we have an error well catch here in the server error or in the coding error before
        res.status(500).send(error);
      });
  } catch (error) {
...
function addFavMoviesHandler(req, res) {
  ...
  let sql = `INSERT INTO movietalbe(title,poster_path,overview,comment) VALUES ('${moviess.title}','${moviess.poster_path}','${moviess.overview}','${moviess.comment}') RETURNING *;`;
